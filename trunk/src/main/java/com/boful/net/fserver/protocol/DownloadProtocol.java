...
public class DownloadProtocol {
    ...
    public static DownloadProtocol parse(IoBuffer ioBuffer) throws IOException {
        ...
        downloadProtocol.setFileSize(fileLength);
        downloadProtocol.setOffset(ioBuffer.getLong());
        downloadProtocol.setLen(ioBuffer.getInt());
        // read hash
        byte[] hashBuffer = new byte[16];
        ioBuffer.get(hashBuffer);
        downloadProtocol.setHash(Hex.encodeHexString(hashBuffer).toUpperCase());

        int remainLen = srcPathLen + destPathLen + downloadProtocol.getLen();
        ...
        byte[] buffer = new byte[downloadProtocol.getLen()];
        ioBuffer.get(srcBuffer);
        ioBuffer.get(destBuffer);
        ioBuffer.get(buffer);

        downloadProtocol.setSrcFile(new File(new String(srcBuffer, "UTF-8")));
        downloadProtocol.setDestFile(new String(destBuffer, "UTF-8"));
